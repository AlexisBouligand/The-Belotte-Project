...
int gameLoop()
{
  ...
  while (points_teams[0] < 300 && points_teams[1] < 300)
  {
...
void startRound(Player_t * player_and_AI, Card_t all_player_and_AI_cards[4][8], char trump)
{
  ...
  for (i = 0; i < 8; i++) //start of tricks
  {
    ...
    displayAfterPlayerChoice( all_player_and_AI_cards[0],table_cards);
  }
  ...
  do
  {
    for(i = 0; i < 4; i++) //for each player
    {
      ...
      if (player_and_AI[i].id == 0)
      {
        ...
        player_and_AI[i].bet = graphicGetBet(&max_bet, all_player_and_AI_cards[0], player_and_AI);// getBet(&max_bet); //get player input for bet
      } else {
      ...
      if (player_and_AI[i].bet > 0) //returned 0 means player has skipped
      {
        ...
        if(player_and_AI[i].id == 0)
        {
          round_trump = graphicGetTrump(); //get the color wanted to be trump
